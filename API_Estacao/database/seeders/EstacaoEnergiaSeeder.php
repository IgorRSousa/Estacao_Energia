            ...
            if($lineReposnse[1] != "NULL"){
                if($dataAnterior != $dataAtual){
                    if($dataAnterior == ""){
                        ...
                        $estacao_energia->Tensao_Fase_RS_V = array_sum($mediaValores[0]) / count($mediaValores[0]);
                        $estacao_energia->Tensao_Fase_ST_V = array_sum($mediaValores[1]) / count($mediaValores[1]);
                        $estacao_energia->Tensao_Fase_TR_V = array_sum($mediaValores[2]) / count($mediaValores[2]);
                        $estacao_energia->Corrente_Fase_R_A = array_sum($mediaValores[3]) / count($mediaValores[3]);
                        $estacao_energia->Corrente_Fase_S_A = array_sum($mediaValores[4]) / count($mediaValores[4]);
                        $estacao_energia->Corrente_Fase_T_A = array_sum($mediaValores[5]) / count($mediaValores[5]);
                        $estacao_energia->Demanda_kW = array_sum($mediaValores[6]) / count($mediaValores[6]);
                        $estacao_energia->Potencia_Ativa_kW = array_sum($mediaValores[7]) / count($mediaValores[7]);
